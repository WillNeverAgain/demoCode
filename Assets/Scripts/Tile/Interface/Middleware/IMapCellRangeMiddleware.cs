...
namespace Tile.Middleware
{
    ...
    /// 通过这个中间件实现访问信息的获取
    /// </summary>
    public interface IMapCellRangeMiddleware : IMapMiddleware
    {
        public enum SearchState
        {
            TRUE,CONTINUE,BRAKE,RETURN
        }
        ...
        /// 返回判断func为真时的tag
        /// </summary>
        public IReadOnlyList<ITagObject> GetObjectWithTagInRange(Vector3 findPosition,Func<IList<string>,bool> tagCharger, IRangeArrayInfo rangeArrayInfo);
        /// <summary>
        ///  返回判断func为真时的tag
        ///  没有顺序和层级，会遍历所有range
        /// </summary>
