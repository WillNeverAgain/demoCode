...
{
    private readonly Dictionary<StatType, AttributeDataUnit<StatContext>> _data;
    private bool _dirty = false;
    public bool AddModifier(StatType type, IValueModifier<StatContext> modifier)
    {
    ...
    public bool RemoveModifier(StatType type, IValueModifier<StatContext> modifier)
    {
        if(!_data.ContainsKey(type)) { return false; }
    ...
    public ReadOnlyDictionary<StatType, AttributeDataUnit<StatContext>> GetValue()
    {
        return new ReadOnlyDictionary<StatType, AttributeDataUnit<StatContext>>(_data);
    }
    ...
    public bool GetDirty(out List<AttributeDataUnit<StatContext>> next_attributes)
    {
        ...
        foreach (StatType attributeDataUnit in _data.Keys)
        {
    ...
    public UnitAttributeData(Dictionary<StatType, AttributeDataUnit<StatContext>> data)
    {
        if(data == null) { data = new Dictionary<StatType, AttributeDataUnit<StatContext>>(); }
        _data = data;
        _dirty = false;
    }
}
