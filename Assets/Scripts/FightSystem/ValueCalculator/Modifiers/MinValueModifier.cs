...
{
    public class MinValueModifier : BaseValueModifier
    {
        ...
        public MinValueModifier(float modifyValue, ValueCalculateStage stage, int priority = 1, string message = null)
        {
        ...
        public override void Modify(ValueCalculateBreakPoint bd)
        {
            if (bd.CurrentValue < modifyValue)
            {
                ...
                bd.Message += ("\nFloor To " + modifyValue + " At Modfy Stage " + Stage.ToString() + "\nCurrent Result Is " + bd.CurrentValue);
            }
        }
    }
}