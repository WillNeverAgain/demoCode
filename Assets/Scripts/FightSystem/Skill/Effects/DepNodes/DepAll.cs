...
{
    /// <summary>
    /// Dependency: All dependencies must be met
    /// </summary>
    public sealed class DepAll : ISkillEffectDependency
    {
        private readonly ISkillEffectDependency[] _deps;
        public DepAll(params ISkillEffectDependency[] deps) => _deps = deps;

        public bool Evaluate(EffectExecuteTrace ctx)
        {
            foreach (var d in _deps)
                if (d != null && !d.Evaluate(ctx)) return false;
            return true;
        }
    }
